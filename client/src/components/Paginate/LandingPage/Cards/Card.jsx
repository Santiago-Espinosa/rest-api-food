...
class Card extends Component {
  delete = async (e) => {
    ...
    const sureToDeleted = window.confirm(
    ...
    if (sureToDeleted) {
      ...
      if (res.status === 500)
        return window.alert("This recipe could not be deleted");
      ...
      <NavLink to={`/recipes/${id}`}>
        <div className={style.card}>
          {!id.toString().includes("-") ? null : (
            <button className={style.delete} onClick={this.delete}>
              X
            </button>
          )}
          <div className={style.nameRecipe}>
            <h2>{name[0].toUpperCase() + name.substr(1)}</h2>
          </div>
          <img src={image} alt={`${id}Img`} />
          <div className={style.scoreConteiner}>
