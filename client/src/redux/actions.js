...
export function getRecipesBackend() {
  return (dispatch) => {
    ...
    axios
      ...
      .catch((err) => {
        window.alert(
          "An error occurred while loading the recipes, please refresh the page"
        );
        dispatch(getDietsBackend());
      });
  };
...
export function getDietsBackend() {
  return (dispatch) => {
    axios
      ...
      .catch((err) => console.log("Diet request to the server failed", err));
  };
}
...
export function getRecipesByName(name) {
  return (dispatch) => {
    ...
    axios
      ...
      .catch((err) =>
        console.log("Request for recipes by name to the server failed", err)
      );
  };
}
