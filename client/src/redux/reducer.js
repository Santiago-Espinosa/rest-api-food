...
const rootReducer = (state = initialState, actions) => {
  switch (actions.type) {
        ...
        newOrder = asc
          ? [
              ...state.allRecipes.sort((a, b) =>
                a[atribute].toLowerCase() > b[atribute].toLowerCase()
                  ? 1
                  : a[atribute].toLowerCase() < b[atribute].toLowerCase()
                  ? -1
                  : 0
    ...
    case "FILTER_BY_DIETS":
      const filteredRecipes = state.backupRecipes.filter(
        (recipe) => !actions.payload.some((d) => !recipe.diets.includes(d))
      );
